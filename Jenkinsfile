...
pipeline {
  ...
  stages {
    ...
    stage('Deploy') {
      steps {
        script {
          // deploy.sh defaults LXD_CONTAINER to secure-vault-<ENV_NAME> when
          // not set; mirror that here so the param can stay blank.
          env.RESOLVED_LXD_CONTAINER = params.LXD_CONTAINER?.trim() ?: "secure-vault-${params.ENV_NAME}"
          echo "Deploying ENV_NAME=${params.ENV_NAME}  LXD_CONTAINER=${env.RESOLVED_LXD_CONTAINER}  SCOPE=${params.SCOPE}"
        }
        sh '''
          set -eu
